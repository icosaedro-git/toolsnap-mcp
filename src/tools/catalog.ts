...
/**
 * Standard note appended to every tool that accepts a URL OR an inline-content
 * argument — points a caller with a local file (no public URL) at the
 * out-of-band POST /upload endpoint instead of pasting the file inline, which
 * costs context with zero storage benefit (the bytes are already tokens you
 * generated by the time they reach the tool call).
 */
const UPLOAD_NOTE =
  "Local file, no public URL? POST the raw bytes to https://mcp.toolsnap.app/upload from your shell (not as inline content) — free ≤25 MB, $0.02 for 25–100 MB with an API key — and pass the returned url. The file never enters your context and auto-deletes within 24h. Inline args are more private (nothing is ever stored) but always cost context; use them only for small files or when you have no shell access.";
...
export const NOTES: Record<string, string> = {
  ...
  html_to_markdown:
    `Provide exactly one of url or html — not both. Do NOT use for JavaScript-rendered SPAs — use fetch_rendered instead. ${HEADERS_NOTE} ${UPLOAD_NOTE}`,
  fetch_metadata:
  ...
  pdf_text_extract:
    `Handles FlateDecode-compressed streams and RC4-encrypted PDFs that open with an empty password. Works on text-based PDFs (Word, LaTeX, web-generated); does NOT perform OCR on scanned/image-only PDFs. Accepts \`data\` (base64) as an inline alternative to \`url\`, capped at 5 MB decoded — not stored, but the base64 still counts against your context; for larger local PDFs, use POST /upload instead. ${HEADERS_NOTE} ${UPLOAD_NOTE}`,
  csv_query:
  ...
  csv_query_xl:
    `Paid sibling of csv_query for URL-hosted CSVs from 5 MB up to 100 MB, streamed server-side (never buffered whole, never touches your context). Same query language, URL-only (no inline csv). $0.02/call or $0.01 prepaid; no first-call-free — it's a paid file-size tier, not a marketing freebie. Billed per query, not per row returned. A local file with no URL can be uploaded first via POST /upload (paid tier covers this size range too), then queried by its returned url. ${HEADERS_NOTE}`,
  json_query:
    `JSONPath-lite: property access, [*] wildcard, [-1] negative index, ..key recursive descent, [?(@.price < 10)] filters. Provide exactly one of url or json. Free up to 5 MB; for larger URL-hosted JSON, see json_query_xl. ${HEADERS_NOTE} ${UPLOAD_NOTE}`,
  json_query_xl:
    `Paid sibling of json_query for URL-hosted JSON from 5 MB up to 25 MB. Same JSONPath-lite query language, URL-only (no inline json). $0.02/call or $0.01 prepaid; no first-call-free. A local file with no URL can be uploaded first via POST /upload, then queried by its returned url. ${HEADERS_NOTE}`,
  sitemap_parse:
  ...
  upload_file:
    "In-band (base64 over MCP): the image bytes ride through your context like any other tool argument — this does not save context, it exists only to hand a local image to remove_background. For csv/json/pdf/html files, or any large file where context matters, use the out-of-band `POST /upload` HTTP endpoint instead (see any file tool's notes) — bytes never touch your context there.",
  image_generate:
